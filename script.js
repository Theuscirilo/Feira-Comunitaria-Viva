...
function initializeHeroAnimations() {
  ...
  elements.forEach((element, index) => {
    ...
    setInterval(() => {
      element.style.transform = `translateY(-20px) rotate(${
        Math.random() * 10 - 5
      }deg)`;
      setTimeout(() => {
...
function initializeScrollAnimations() {
  ...
  const animatedElements = document.querySelectorAll(
    ".help-card, .gallery-item, .testimonial-card, .stat, .visual-card"
  );
  ...
  socialLinks.forEach((link) => {
    link.addEventListener("click", (e) => {
      ...
      if (icon.classList.contains("fa-facebook")) {
        ...
        window.open(
          "https://www.instagram.com/theussft_/",
          "_blank"
        );
      } else if (icon.classList.contains("fa-twitter")) {
        window.open("https://x.com/theusszin_", "_blank");
      } else if (icon.classList.contains("fa-whatsapp")) {
        window.open(
          "https://wa.me/5543996825923?text=Olá! Gostaria de saber mais sobre a Feira Comunitária Viva.",
          "_blank"
        );
      }
    });
